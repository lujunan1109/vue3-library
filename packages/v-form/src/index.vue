    ...
    <!-- 单独渲染没有嵌套结构 -->
      <div v-for="(formItem,inx) in formOptions" :key="inx">
        <el-form-item :label="formItem.label" :prop="formItem.prop">
          <component :is="renderObj[formItem.componentName]" v-model="ruleForm[formItem.prop]" v-bind="formItem.attrs">
              <div v-if="formItem.childName">
                <component v-for="(child) in formItem.children" :key="child.value" :is="renderObj[formItem.childName]" :label="child.label" :value="child.value">
      ...
      <!-- 动态名称自定义插槽 -->
      <slot :customSlot="formItem.slotName"></slot>
      </div>
...
import { ElInput, ElButton, ElSelect, ElOption, ElDatePicker, ElCascader, ElSwitch, ElRadio, ElCheckbox, ElCheckboxGroup } from 'element-plus';
